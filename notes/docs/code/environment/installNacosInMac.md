...
MYSQL_SERVICE_HOST=MYSQL所在的IP地址
...
### 拉取镜像并且创建容器
```shell
(base) wangwenpeng@AppleBook microService % docker run -d \
--name nacos \
--env-file ./nacos/custom.env \ #环境配置文件
-p 8848:8848 \ #用于客户端与服务通信的主要端口。
-p 9848:9848 \ #Nacos 的 gRPC 通信
-p 9849:9849 \ #Nacos 集群中节点之间的通信
nacos/nacos-server:latest
```

网络环境允许的可以直接拉取最新的网络镜像。或者可以自己从网络下载镜像`nacosxxx.tar`，然后执行`docker load -i nacosxxx.tar`加载之后，再创建容器。
